...
Ytrain = ytr
print("Train-set length : ",trLen , " : ",len(paperTrain) , " : ",len(revTrain) , " : ", len(Ytrain))
...
pra = concatenate([pa , rev_embedded] , axis=-1)
praV = TimeDistributed(TimeDistributed(Dense(1,activation="sigmoid")))(pra)
praV_out = TimeDistributed(Flatten())(praV)
prav = TimeDistributed(RepeatVector(rev_embed_dim))(praV_out)
prav = Permute((1,3,2))(prav)

pr = Multiply()([prav , rev_embedded])
pr = Reshape((3,rev_mx_snts,rev_embed_dim,1))(pr)
...
model = Model(inputs=[paper_input, rev_input] , outputs=mlp_out)
...
for i in range(num_epochs):
	...
	model.fit([paperTrain , revTrain], Ytrain, batch_size = batch_sz, shuffle=False,epochs = 1,verbose=2)
	tmp = model.predict([paperTest , revTest])
	#
	mat = [[0,0],[0,0]]
	for i in range(teLen):
		yp = 0
